...
const SplashScreen = ({ onFinish }: { onFinish: () => void }) => {
  ...
  useEffect(() => {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // FLOATERS — zwiększona liczba (100), większe rozmiary (0.8–3.5px),
    // wyższa widoczność (opacity 0.25–0.75), wolniejszy ruch, trzy strefy hue:
    //   warm orange/amber (hue 18–42) dla ogółu,
    //   jasno-żółte akcenty (hue 42–55) co piąta kulka,
    //   kremowo-białe (hue 30, sat 20%) co dziesiąta — efekt "iskier"
    // ─────────────────────────────────────────────────────────────────────────
    const floaters = Array.from({ length: 100 }, (_, i) => {
      const isAccent = i % 5 === 0; // jaśniejsza, większa kulka
      const isSpark = i % 10 === 0; // kremowa iskra
      return {
        x: Math.random(),
        y: Math.random() + 0.05,
        ...
        r: isSpark
          ? Math.random() * 1.2 + 2.2 // iskry: 2.2–3.4 px
          : isAccent
            ? Math.random() * 1.0 + 1.6 // akcenty: 1.6–2.6 px
            : Math.random() * 1.0 + 0.8, // bazowe: 0.8–1.8 px
        op: isSpark
          ? Math.random() * 0.25 + 0.55 // iskry: 0.55–0.80
          : isAccent
            ? Math.random() * 0.2 + 0.45 // akcenty: 0.45–0.65
            : Math.random() * 0.25 + 0.25, // bazowe: 0.25–0.50
        ph: Math.random() * Math.PI * 2,
        phSpeed: Math.random() * 0.008 + 0.006, // wolniejsze pulsowanie
        hue: isSpark
          ? 38 + Math.random() * 14 // kremowo-złote
          : isAccent
            ? 28 + Math.random() * 16 // ciepły pomarańcz
            : 16 + Math.random() * 24, // głębszy amber
        sat: isSpark
          ? 40 + Math.random() * 20 // desaturowane → kremowe
          : 70 + Math.random() * 20, // nasycone
        lit: isSpark
          ? 82 + Math.random() * 12 // jasne iskry
          : isAccent
            ? 70 + Math.random() * 12
            : 62 + Math.random() * 16,
      };
